...
/// index and the third one the hadronic tau index.
auto TripleSelectionAlgo(const float &mindeltaR_lep1lep1) {
        Logger::get("lep1lep1_lep1::TripleSelectionAlgo")
            ->debug("Setting up algorithm");
        return [mindeltaR_lep1lep1](const ROOT::RVec<float> &lep1_pt,
                                    const ROOT::RVec<float> &lep1_eta,
                                    const ROOT::RVec<float> &lep1_phi,
                                    const ROOT::RVec<float> &lep1_mass,
                                    const ROOT::RVec<float> &lep1_iso,
                                    const ROOT::RVec<int> &lep1_charge,
                                    const ROOT::RVec<int> &lep1_mask) {
            // Entries are lep1 indices
            ROOT::RVec<int> selected_triple = {-1, -1, -1};
            const auto ind_good_lep1 = ROOT::VecOps::Nonzero(lep1_mask);
            Logger::get("lep1lep1_lep1::TripleSelectionAlgo")
                ->debug("masks: org lep1 {}", lep1_mask);

            // Only select events with at least three good lep1
            if (ind_good_lep1.size() < 3) {
                return selected_triple;
            }
            ...
            // Sort lep1 candidates by isolation
            const auto good_lep1_iso = ROOT::VecOps::Take(lep1_iso, ind_good_lep1);
            const auto sorted_good_lep1_idx = ROOT::VecOps::Argsort(good_lep1_iso);

            // Select the first three leptons
            for (int i = 0; i < selected_triple.size(); ++i) {
                selected_triple[i] = ind_good_lep1[sorted_good_lep1_idx[i]];
            }

            // Check deltaR conditions if needed
            // ...
 ...
 * @param df the input dataframe
 * @param input_vector vector of strings containing the columns needed for the
 * alogrithm. For the mumumu triple selection these values are:
    - muon_pts
    - muon_etas
    - muon_phis
    - muon_masses
...
ROOT::RDF::RNode TripleSelection(ROOT::RDF::RNode df,
                                 ...
                                 const std::string &triplename,
                                 const float &mindeltaR_lep1lep1) {
    Logger::get("mumumu::TripleSelection")
        ->debug("Setting up MuMuMu Triple building");
    auto df1 =
        df.Define(triplename,
                  whtautau_tripleselection::lep1lep1_lep1::TripleSelectionAlgo(
                      mindeltaR_lep1lep1),
                  input_vector);
    return df1;
}
} // end namespace mumumu
